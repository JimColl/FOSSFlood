...
suppressMessages(suppressWarnings(library(dataRetrieval)))
suppressMessages(suppressWarnings(library(webshot)))
suppressMessages(suppressWarnings(library(magick)))
suppressMessages(suppressWarnings(library(animation)))
...
suppressMessages(suppressWarnings(library(tigris)))
suppressMessages(suppressWarnings(library(cdlTools)))
suppressMessages(suppressWarnings(library(httr)))
suppressMessages(suppressWarnings(library(openadds)))
suppressMessages(suppressWarnings(library(plainview)))
suppressMessages(suppressWarnings(library(tidygeocoder)))
...
suppressMessages(suppressWarnings(library(shinybusy)))
suppressMessages(suppressWarnings(library(velox)))
...
raster::rasterOptions(datatype="INT4S")
# raster::rasterOptions(datatype="INT4U")
# raster::rasterOptions(datatype="FLT4S")

apptitle = "FOSSFlood V 1.25"
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
if (!file.exists(paste0(basedir, "/data/misc/zctas.shp"))) {
  ...
  httr::GET('https://www2.census.gov/geo/tiger/TIGER2010/ZCTA5/2010/tl_2010_us_zcta510.zip', write_disk(paste0(basedir, "/data/misc/tl_2010_us_zcta510.zip")), overwrite=TRUE)
  unzip(paste0(basedir, "/data/misc/tl_2010_us_zcta510.zip"), exdir = paste0(basedir, "/data/misc"))
  zctasShape <- sf::read_sf(paste0(basedir, "/data/misc/tl_2010_us_zcta510.shp"))
  data.table::set(zctasShape, j=c("CLASSFP10",'MTFCC10','FUNCSTAT10','ALAND10','AWATER10','INTPTLAT10','INTPTLON10'), value=NULL)
  # zctasShapeProj <- sf::st_transform(sf::st_as_sf(zctasShape), sf::st_crs(4326))
  zctasShapeProj <- sf::st_transform(sf::st_as_sf(zctasShape), sf::st_crs(3857))
  sf::write_sf(sf::st_as_sf(zctasShapeProj), paste0(basedir, "/data/misc/zctas.shp"), delete_layer = TRUE, quiet = TRUE)
  unlink(paste0(basedir, "/data/misc/tl_2010_us_zcta510.dbf"))
  unlink(paste0(basedir, "/data/misc/tl_2010_us_zcta510.shx"))
  unlink(paste0(basedir, "/data/misc/tl_2010_us_zcta510.shp.xml"))
  unlink(paste0(basedir, "/data/misc/tl_2010_us_zcta510.shp"))
  unlink(paste0(basedir, "/data/misc/tl_2010_us_zcta510.prj"))
  unlink(paste0(basedir, "/data/misc/tl_2010_us_zcta510.zip"), recursive = TRUE)
}
if (!file.exists(paste0(basedir, "/data/misc/huc8.shp"))) {
  print("-- Unpacking FOSSFlood for first use: Downloading HUC units (this may take a while)--")
  httr::GET('https://prd-tnm.s3.amazonaws.com/StagedProducts/Hydrography/WBD/National/GDB/WBD_National_GDB.zip', 
            write_disk(paste0(basedir, "/data/misc/WBD_National_GDB.zip")), overwrite=TRUE)
  ...
  HUC8ShapeProj <- sf::st_transform(HUC8Shape, sf::st_crs(3857))
...
if(user.aoi.source %in% c("zctas", "huc8")) {
  user.aoi.stringlist <- str_sort(unlist(as.list(strsplit(user.aoi.string, ", ")[[1]])))
  if(!identical(user.aoi.string,user.aoi.stringlist)) {
    user.aoi.string <- paste(as.character(user.aoi.stringlist),collapse=", ",sep="")
    print(paste("-- Welcome to FOSSFlood - sorted request as", user.aoi.string))
  }  
  ...
  userPoint4326 <- sf::st_as_sf(tidygeocoder::geo(address=user.aoi.string,method = "osm"), coords = c("long", "lat"), crs = 4326)
  userBB4326 <- sf::st_bbox(sf::st_buffer(userPoint4326,1500,nQuadSegs = 30,endCapStyle = "ROUND",joinStyle = "ROUND",mitreLimit = 1,singleSide = FALSE))  
  user.aoi.call <- sf::st_transform(sf::st_as_sfc(userBB4326), sf::st_crs(3857))
  # user.aoi.stringlist <- as.list(strsplit(user.aoi.filepath <-, ", ")[[1]])
  user.aoi.filepath <- gsub(", ", "_", user.aoi.string)
  user.aoi.filepath <- gsub(" ", "_", user.aoi.filepath)
}

if(length(user.aoi.call$geometry)==0){
  print("-- Requested AOI not found, double check that your features exist and that you entered them correctly. --")
  stop()
}

# Run AOI and Floodmapping packages
# AOI = AOI::aoi_get(user.aoi.call)
# tryCatch({quiet(geosphere::areaPolygon(sf::as_Spatial(AOI$sf..st_as_sfc.x.))>0)}, 
#          error = function(e) {
#            print("-- Malformed input: The features were not located in the AOI.  Double check that your features exist and that you entered them correctly.")
#            stop(e)
#          })
# print(paste("-- Welcome to FOSSFlood - Processing", round(geosphere::areaPolygon(sf::as_Spatial(AOI$sf..st_as_sfc.x.))/2.59e+6,2),"square miles"))
print(paste("-- Welcome to FOSSFlood - Processing", round(rgeos::gArea(sf::as_Spatial(user.aoi.call$geometry))/2.59e+6,2),"square miles"))
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
# ----- Gather or Generate HAND flood mapping data  ----------------------------------------------------------------------------------
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
if(file.exists(paste0(basedir,"/AOI/",user.aoi.filepath,"/grid_rec.shp"))) {
  xx = list(hand.path = paste0(basedir,"/AOI///",user.aoi.filepath,"/hand_",user.aoi.filepath,".tif"),
            ...
            aoi.path = paste0(basedir,"/AOI/",user.aoi.filepath,"/",user.aoi.filepath,"_hardclip.shp"))
} else {
  print("-- Welcome to FOSSFlood - Prepping base data, this can take a while to process so please be patient and unless an explicit error appears be assured it's running")
  
  # Create vis list and generate folder structure for requested AOI
  xx <- list() 
  ...
  tmpdir <- paste0(basedir,"/AOI/",user.aoi.filepath,"/tmp/")
  
  sf::write_sf(user.aoi.call, paste0(basedir,"/AOI/",user.aoi.filepath,"/",user.aoi.filepath,"_hardclip.shp"), delete_layer = TRUE, quiet = TRUE)
  xx$aoi.path <- paste0(basedir,"/AOI/",user.aoi.filepath,"/",user.aoi.filepath,"_hardclip.shp")
  
  xx$aoi.shp <- sf::read_sf(xx$aoi.path) %>% sf::st_transform(sf::st_crs(4326))
  xx$aoi.bb <- sf::st_as_sfc(sf::st_bbox(xx$aoi.shp))
  south <- sf::st_bbox(xx$aoi.shp)[2]
  north <- sf::st_bbox(xx$aoi.shp)[4]
  west <- sf::st_bbox(xx$aoi.shp)[1]
  east <- sf::st_bbox(xx$aoi.shp)[3]
  
  HUC8_Shapefile <- sf::read_sf(paste0(basedir, "/data/misc/huc8.shp"))[user.aoi.call,]
  HUC8Labels <- as.character(HUC8_Shapefile$huc8)
  GrabHUC6Labels_func <- function(HUC8LabelString) {return( stringr::str_sub(HUC8LabelString, 1,6) )}
  HUC6_labels_toprocess <- sapply(HUC8Labels,GrabHUC6Labels_func) %>% unique()
  
  if(length(HUC6_labels_toprocess)<=0) {
    print("-- Requested AOI not within the processed CFIM dataset, check the map at (URL HERE) for valid areas, sorry :( --")
    stop()
  }
  
  for(i in 1:length(HUC6_labels_toprocess)) {
    # if it's already unwrapped
    if(file.exists(paste0(basedir,'/AOI/',HUC6_labels_toprocess[i],'/',HUC6_labels_toprocess[i],'dd.tif'))) {
      print(paste0("-- Welcome to FOSSFlood - ",HUC6_labels_toprocess[i], "already exists"))
      
      # If it's downloaded but not unwrapped
    } else if(file.exists(paste0(basedir,'/AOI/',HUC6_labels_toprocess[i],'.zip'))) {
      print(paste0("-- Welcome to FOSSFlood - Unzipping HUC ",HUC6_labels_toprocess[i]))
      unzip(paste0(basedir,'/AOI/',HUC6_labels_toprocess[i],'.zip'), exdir = paste0(basedir,'/AOI')) 
      
      # Otherwise download and unzip
    } else {
      print(paste0("-- Welcome to FOSSFlood - Downloading and unzipping HUC ",HUC6_labels_toprocess[i]))
      httr::GET(paste0('https://cfim.ornl.gov/data/HAND/20200301/',HUC6_labels_toprocess[i],'.zip'),
                write_disk(paste0(basedir,'/AOI/',HUC6_labels_toprocess[i],'.zip')), overwrite=TRUE)
      unzip(paste0(basedir,'/AOI/',HUC6_labels_toprocess[i],'.zip'), exdir = paste0(basedir,'/AOI'))   
    }
  }
  ...
  print("-- Welcome to FOSSFlood - merging CATCH data")
  openCATCHRasters_func <- function(HUC6LabelsString) {return( raster::raster(paste0(basedir,'/AOI/',HUC6LabelsString,'/',HUC6LabelsString,'catchmask.tif')) )}
  all_CATCHrasters <- lapply(HUC6_labels_toprocess,openCATCHRasters_func)
  if(length(HUC6_labels_toprocess)>1) {
    merged_CATCHrasters <- do.call(raster::merge, all_CATCHrasters)
  } else {
    merged_CATCHrasters <- all_CATCHrasters[[1]]
  }
  print("-- Welcome to FOSSFlood - cropping CATCH data")
  raster::crop(x = merged_CATCHrasters, y = raster::extent(user.aoi.call %>% sf::st_transform(sf::st_crs(raster::crs(merged_HANDRasters)))), snap="in", filename=paste0(tmpdir,"CATCH_crop.tif"))
  print("-- Welcome to FOSSFlood - projecting CATCH data")
  gdalUtilities::gdalwarp(srcfile = paste0(tmpdir,"CATCH_crop.tif"), dstfile = paste0(basedir,"/AOI/",user.aoi.filepath,"/catchmask_",user.aoi.filepath,".tif"), t_srs = sf::st_crs(3857), r = "near")   
  xx$hand.path <- paste0(basedir,"/AOI/",user.aoi.filepath,"/hand_",user.aoi.filepath,".tif")
  xx$catch.path <- paste0(basedir,"/AOI/",user.aoi.filepath,"/catchmask_",user.aoi.filepath,".tif")
  # ---------------------------------------------------------------------------------
  valid_comid_values <- raster::unique(raster::raster(paste0(basedir,"/AOI/",user.aoi.filepath,"/catchmask_",user.aoi.filepath,".tif")))
  
  
  # Rating Curves  ------------------------------------------------------------------
  print("-- Welcome to FOSSFlood - creating rating curves")
  rating_curves = list()
  openratingcurve_func <- function(HUC6LabelsString) {
    build <- read.csv(paste0(basedir,'/AOI/',HUC6LabelsString,'/hydrogeo-fulltable-',HUC6LabelsString,'.csv'))
    build <- cbind(build[,1], build[,2], build[,15])
    build <- build[valid_comid_values %in% build[,1],]
    return(build)
  }
  all_ratings <- lapply(HUC6_labels_toprocess,openratingcurve_func)
  all_ratings_df <- as.data.frame(do.call(rbind, lapply(all_ratings, as.data.frame)))
  colnames(all_ratings_df) <- c("COMID", "Y", "Qcms")
  fst::write_fst(all_ratings_df,paste0(basedir,"/AOI/",user.aoi.filepath,"/rating_",user.aoi.filepath,".fst"))
  xx$rating.path <- paste0(basedir,"/AOI/",user.aoi.filepath,"/rating_",user.aoi.filepath,".fst")
  
  # ---------------------------------------------------------------------------------
  print("-- Welcome to FOSSFlood - merging, projecting, and clipping flowlines")
  openFlowLines_func <- function(HUC6LabelsString) {return( sf::read_sf(paste0(basedir,'/AOI/',HUC6LabelsString,'/',HUC6LabelsString,'-flows.shp')) )}
  FlowLinesList <- lapply(HUC6_labels_toprocess,openFlowLines_func)
  all_flowlines <- do.call(rbind, FlowLinesList)
  print("-- Welcome to FOSSFlood - projecting flowlines")
  all_flowlines_proj <- sf::st_transform(all_flowlines, sf::st_crs(3857)) %>% sf::st_zm(drop = TRUE)
  all_flowlines_valid <- all_flowlines_proj[user.aoi.call,]
  flowlines_namesToKeep <- names(all_flowlines_proj) %in% 
    c("COMID", "GNIS_NAME", "FTYPE","StreamOrde", "StreamLeve", "StreamCalc", "geometry")
  sf::write_sf(all_flowlines_valid[,flowlines_namesToKeep], paste0(basedir,"/AOI/",user.aoi.filepath,"/nhdflowline_network.shp"), delete_layer = TRUE, quiet = TRUE)
  # ---------------------------------------------------------------------------------
  ...
  tryCatch(quiet(httr::GET(URL, write_disk(paste0(tmpdir,"inventory"), overwrite=TRUE))),
           error = function(e) {
  ...
  if(!skipNWISflag) {
    NWISgagestxt <- read.table(paste0(tmpdir,"inventory"), sep="\t", header=TRUE)[-1,]  # Remove colume data def
    ...
    NWISgages_proj <- do.call(cbind, list(NWISgages_proj, NWISgageCOMID))
    suppressWarnings(sf::write_sf(NWISgages_proj, paste0(basedir,"/AOI/",user.aoi.filepath,"/NWISgages.shp"), delete_layer = TRUE, quiet = TRUE))
  }
  # ---------------------------------------------------------------------------------
  ...
  CountiesFIPS_Proj <- sf::st_transform(sf::st_as_sf(CountiesFIPS), sf::st_crs(3857))
  aoiCounties <- suppressMessages(CountiesFIPS_Proj[user.aoi.call, ])
  ...
  tigerroads_Proj <- sf::st_transform(sf::st_as_sf(tigerroads), sf::st_crs(3857))
  tigerroads_Proj_Sub <- suppressMessages(tigerroads_Proj[user.aoi.call, ])
  ...
  for(t in 1:length(unique(aoiCounties$STATEFP))){
    ...
    tryCatch({
      quiet(httr::GET(paste0("http://download.geofabrik.de/north-america/us/",tolower(gsub(" ", "-", cdlTools::fips( unique(aoiCounties$STATEFP)[t], to = "Name"))),"-latest.osm.pbf"), 
                      write_disk(paste0(tmpdir,tolower(gsub(" ", "-", cdlTools::fips(unique(aoiCounties$STATEFP)[t], to = "Name"))),"-latest.osm.pbf"), 
                                 overwrite=TRUE)))
    },
    error=function(cond) {
    ...
    OSMlines <- read_from_gpkg(paste0(tmpdir,tolower(gsub(" ", "-", cdlTools::fips(unique(aoiCounties$STATEFP)[t], to = "Name"))),"-latest.osm.pbf"), "lines")
    OSMlines_Proj <- sf::st_transform(sf::st_as_sf(OSMlines), sf::st_crs(3857))
    OSMlines_Proj_Sub <- suppressMessages(OSMlines_Proj[user.aoi.call, ])
    ...
    OSMpoints <- read_from_gpkg(paste0(tmpdir,tolower(gsub(" ", "-", cdlTools::fips(unique(aoiCounties$STATEFP)[t], to = "Name"))),"-latest.osm.pbf"), "points")
    OSMpoints_Proj <- sf::st_transform(sf::st_as_sf(OSMpoints), sf::st_crs(3857))
    OSMpoints_Proj_Sub <- suppressMessages(OSMpoints_Proj[user.aoi.call, ])
    ...
    tryCatch({
      ...
      sfoadata <- sf::st_as_sf(mergedStateData) %>% sf::st_set_crs(3857)
      print(paste0("-- Trimming OpenAddressess - may be slow depending on the size of the database --"))
      aoiPointBounds <- suppressWarnings(suppressMessages(sfoadata[user.aoi.call, ]))
  ...
  hex_grid <- make_grid(user.aoi.call, type = "hexagonal", cell_area = 2250000, clip = FALSE)
  rec_grid <- make_grid(user.aoi.call, type = "square", cell_area = 2250000, clip = FALSE)
  ...
  if(file.exists(paste0(basedir,"/AOI/",user.aoi.filepath,"/addresses_oa.shp"))) {
    if(length(aoiPoints$geometry)==0) {
      print(paste0("Errors encountered generating Openaddresses, database empty (bad AOI or too large to process in one go)"))
    } else {
  ...
  if(file.exists(paste0(basedir,"/AOI/",user.aoi.filepath,"/nhdflowline_network.shp"))) {
    print(paste0("No errors generating NHD flowline data: ",nrow(all_flowlines_valid)," segments equivalent to ",toString(sum(sf::st_length(all_flowlines_valid))*0.000621371)," miles of waterways in AOI")) } else { print("Issues downloading flow lines") }
  ...
  if(all(
    ...
    paste("-- No critical errors generating base data - Map on! --")
  }
  ...
  capture.output(
    cat(
      ...
      "Number of HAND Cells:", suppressWarnings(suppressMessages(raster::ncell(raster::raster(xx$hand.path)))),"\n\n",
      "Address points:\n",
      ...
      "NHD flowline data:\n",
      nrow(all_flowlines_valid),"segments equivalent to",toString(sum(sf::st_length(all_flowlines_valid))*0.000621371),"miles of waterways in AOI"
    ),
    file = paste0(basedir,"/AOI/",user.aoi.filepath,"/AOI_Report.txt")
  )
  
  unlink(paste0(basedir,"/AOI/",user.aoi.filepath,"/tmp"), recursive=TRUE)
  unlink_func <- function(HUC6LabelsString) {
    paste0(basedir,'/AOI/',HUC6LabelsString,'.zip')
    unlink(paste0(basedir,"/AOI/",HUC6LabelsString),recursive=TRUE)
  }
  lapply(HUC6_labels_toprocess,unlink_func)
  quiet(gc())
}
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
xx$aoi.bb <- sf::st_as_sfc(sf::st_bbox(xx$aoi.shp))
xx$catch.grid = suppressWarnings(raster::raster(xx$catch.path))
xx$hand.grid = suppressWarnings(raster::raster(xx$hand.path))
# xx$catch.grid = stars::read_stars(xx$catch.path)
# xx$hand.grid = stars::read_stars(xx$hand.path)
...
if(length(xx$address.point$geometry)==0){
  ...
  print(paste0("-- !ALERT! selected address database is empty, swapping to ",user.address.source," --"))
}
...
tryCatch(xx$road.line <- sf::read_sf(xx$road.path),
         error = function(e) {
           print("-- !ALERT! Invalid roads provided, defaulting to TIGER database --")
           xx$road.path <<- xx$tigerroads.path
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
# ----- Load in NOMADS NWM SR Flows ----------------------------------------------------------------------------
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
# -- Grab NWM ------------------------------------------------------------------------------------------------------------
print("-- Grabbing NWM forecasts -- Downloading Forecasts: This may take a few minutes")

dir.create(paste0(basedir,"/AOI/NWM"))
unlink(paste0(basedir,"/AOI/",user.aoi.filepath,"/flows_",user.aoi.filepath,".fst"))
...
for(n in 1:user.forecast.timesteps) {
  print(paste0("-- Grabbing NWM forecasts -- Downloading Forecast for hour ",n," of ",user.forecast.timesteps))
  httr::GET(paste0('http://nomads.ncep.noaa.gov/pub/data/nccf/com/nwm/prod/nwm.', fileDate,'/short_range/nwm.t',rtime, 'z.short_range.channel_rt.f',str_pad(n, 3, pad = "0"),'.conus.nc'), 
            write_disk(paste0(basedir,"/AOI/NWM/nwm.t",rtime, 'z.short_range.channel_rt.f',str_pad(n, 3, pad = "0"),'.conus.nc'), 
                       overwrite=TRUE))
}

# --Grab the COMIDs ------------------------------------------------------------------------------------------------------------
print("-- Grabbing NWM forecasts -- Subsetting NWM files to selected reaches")
comid_to_process <- unique(x=xx$catch.grid)
# comid_to_process <- unique(as.vector(xx$catch.grid %>% pull))
...
nc <- ncdf4::nc_open(filename = paste0(folder, files[[1]]))
vars <- nc$var
test1test2 <- vars$streamflow$dim[[1]]$vals
comids_of_value <- comid_to_process[comid_to_process %in% test1test2]
...
nwm.flow[is.na(nwm.flow)] <- 0
# nwm.flow = as.data.frame(nwm.flow) %>% add_column(comids_of_value, .before = 1)

#rownames(nwm.flow) <- comids_of_value

firstDateTime <- strptime(paste0(fileDate," ",rtime,':00:00'), format ="%Y%m%d %H:%M:%S",tz = "UTC")
# newColumnNames <- list('COMID', paste0('t_',format(firstDateTime,"%Y-%m-%d %H:%M:%S")))
newColumnNames <- list(paste0('t_',format(firstDateTime,"%Y-%m-%d %H:%M:%S")))

for(t in 1:(user.forecast.timesteps-1)) {
  newColumnNames <- c(newColumnNames, paste0('t_',format(firstDateTime + lubridate::hours(t),"%Y-%m-%d %H:%M:%S")))
}
#newColumnNames
colnames(nwm.flow) <- newColumnNames
row.names(nwm.flow) <- comids_of_value
nwm.flow <- as.data.frame(nwm.flow)

nwm.flow <- cbind(rownames(nwm.flow), nwm.flow)
rownames(nwm.flow) <- NULL
colnames(nwm.flow)[1] <- "COMID"

fst::write.fst(nwm.flow,paste0(basedir,"/AOI/",user.aoi.filepath,"/flows_",user.aoi.filepath,".fst"))
print("-- Grabbing NWM forecasts -- Flows written")

unlink(paste0(basedir,"/AOI/NWM"), recursive=TRUE)
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
user.forecast.gen <- names(nwm.flow[2]) %>% stringr::str_replace_all("t","") %>% stringr::str_replace_all("_","") %>% stringr::str_replace_all("-","")  %>% stringr::str_replace_all(" ","")  %>% stringr::str_replace_all(":","") %>% substr(1, 10)


#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
# ----- Pull time series out of flow header --------------------------------------------------------------------
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
# if(is in nwm) {
xx$nwm.discharge <- fst::read.fst(paste0(basedir,"/AOI/",user.aoi.filepath,"/flows_",user.aoi.filepath,".fst"))
xx$nwm.discharge.dateTimeZoneZ <- names(xx$nwm.discharge)[-1] %>% stringr::str_sub(3, 21) %>% as.POSIXct(tz="UTC")
xx$nwm.discharge.dateTimeZoneLocal <- as.POSIXct(xx$nwm.discharge.dateTimeZoneZ,tz=Sys.timezone())
...
# ----- CFIM flow to stage ----------------------------------------------------------------------------------
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
print("-- CFIM flow forecasts to stage --")

j = NULL
`%dopar%` = foreach::`%dopar%`

flows <- fst::read.fst(paste0(basedir,"/AOI/",user.aoi.filepath,"/flows_",user.aoi.filepath,".fst"))
rating_curves <- fst::read.fst(paste0(basedir,"/AOI/",user.aoi.filepath,"/rating_",user.aoi.filepath,".fst"))
catchmentv = suppressWarnings(velox::velox(xx$catch.path))
handv = suppressWarnings(velox::velox(xx$hand.path))
...
COMID = unique(flows$COMID)
stage = NULL

for(i in seq_along(COMID)){
  flow = flows[flows$COMID == COMID[i],]
  curve = rating_curves[rating_curves$COMID == COMID[i],]
  fin = NULL
  
  for(j in 2:dim(flow)[2]){
    tmp = curve$Y[which.min(abs(curve$Qcms - flow[1,j]))]
...
stage = cbind(COMID, stage)
rownames(stage) = NULL
colnames(stage) = c('COMID',newColumnNames)
stage = as.data.frame(stage, stringsAsFactors = FALSE)
stage$COMID <- as.numeric(stage$COMID)
...
# catch.v = as.vector(catchmentv$extract(sp=xx$aoi.bb, fun=NULL))
# hand.v = as.vector(catchmentv$extract(sp=xx$aoi.bb, fun=NULL))
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
# ----- Generate Flood rasters ----------------------------------------------------------------------------------
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
print("-- Creating Flood Inundation Rasters --")
...
print("-- Creating Flood Inundation Rasters -- Matching stage to catchments  --")
a <- foreach::foreach(j = 2:NCOL(stage), .combine = raster::stack) %dopar% {
  val.v = as.numeric(stage[fastmatch::fmatch(catch.v, stage$COMID), j])
  fin.v = val.v - hand.v
  fin.v[fin.v <= 0] <- NA
  #fin.v[fin.v > 0] <- 1
...
xx$flood.grid <- round(a, digits = 3)
# names(xx$flood.grid) <- xx$nwm.discharge.dateTimeZoneZHR %>% str_replace_all(":", "_")
names(xx$flood.grid) <- colnames(nwm.flow)[-1]
...
if(user.output.choice=="GIS_O") {
  ...
  exportnames <- paste0(basedir,"/AOI/",user.aoi.filepath,"/output/",exportnames)
  raster::writeRaster(xx$flood.grid, filename=exportnames, bylayer=TRUE,format="GTiff")
  print(paste0("-- Flood inundation rasters written to: ",basedir,"/AOI/",user.aoi.filepath,"/output/ --"))
  if(user.output.choice=="GIS_O") {
...
xx$address.impactMapping = v$extract_points(xx$address.point)
colnames(xx$address.impactMapping) <- names(xx$flood.grid)
xx$address.point <- do.call(cbind, list(xx$address.point, xx$address.impactMapping))
...
if(all(xx$address.point$ffreq==0,na.rm = TRUE) | user.output.choice=="basedata") {
  ...
  if(substr(user.forecast.source, 1, 8)=="USER_DIS" || substr(user.forecast.source, 1, 10)=="USER_STAGE") {
    ...
    forecastZ <- ISOdatetime(year=substr(user.forecast.gen, 1, 4),
                             month=substr(user.forecast.gen, 5, 6),
                             day=substr(user.forecast.gen, 7, 8),
                             hour=substr(user.forecast.gen, 9, 11),
                             min="00",
                             sec="00",
                             tz="GMT") 
  ...
  xx$mapindex.point <- suppressWarnings(suppressMessages(xx$mapindex.poly %>% sf::st_centroid()))
  ...
  print("--  Projecting vector data for mapping --")
  xx$aoi.shp <- sf::st_transform(xx$aoi.shp, sf::st_crs(4326))
  xx$aoi.bb <- sf::st_transform(xx$aoi.bb, sf::st_crs(4326))
  xx$gage.point <- xx$gage.point %>% sf::st_transform(sf::st_crs(4326))
  xx$address.point <- xx$address.point %>% sf::st_transform(sf::st_crs(4326))
  xx$road.line <- xx$road.line %>% sf::st_transform(sf::st_crs(4326))
  xx$road.point <- xx$road.point %>% sf::st_transform(sf::st_crs(4326))
  xx$flow.line <- xx$flow.line %>% sf::st_transform(sf::st_crs(4326))
  xx$flow.point <- xx$flow.point %>% sf::st_transform(sf::st_crs(4326))
  xx$mapindex.poly <- xx$mapindex.poly %>% sf::st_transform(sf::st_crs(4326))
}
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
if(processskipflag) {
  ...
  xx$road.impactMapping = v$extract_points(xx$road.point)
  colnames(xx$road.impactMapping) <- names(xx$flood.grid)
  xx$road.point =  do.call(cbind, list(xx$road.point, xx$road.impactMapping))
  xx$road.point.mapbook = suppressWarnings(suppressMessages(sf::st_intersection(xx$mapindex.poly, xx$road.point)))
  ...
  print("-- Projecting vector data for mapping --")
  xx$aoi.shp <- sf::st_transform(xx$aoi.shp, sf::st_crs(4326))
  xx$aoi.shp_d <- sf::st_transform(xx$aoi.shp_d, sf::st_crs(4326))
  xx$aoi.bb <- sf::st_transform(xx$aoi.bb, sf::st_crs(4326))
  xx$gage.point <- sf::st_transform(xx$gage.point, sf::st_crs(4326))
  xx$address.point <-sf::st_transform(xx$address.point, sf::st_crs(4326))
  xx$address.mapbook <-sf::st_transform(xx$address.mapbook, sf::st_crs(4326))
  xx$road.line <- sf::st_transform(xx$road.line, sf::st_crs(4326))
  xx$road.point <- sf::st_transform(xx$road.point, sf::st_crs(4326))
  xx$road.point.mapbook <- sf::st_transform(xx$road.point.mapbook, sf::st_crs(4326))
  xx$flow.line <- sf::st_transform(xx$flow.line, sf::st_crs(4326))
  xx$flow.point <- sf::st_transform(xx$flow.point, sf::st_crs(4326))
  xx$mapindex.poly <- sf::st_transform(xx$mapindex.poly, sf::st_crs(4326))
  ...
  # Human readable field names
  xx$mapindex.poly <- xx$mapindex.poly %>% 
    dplyr::rename(`Index Label` = IndexLabel) %>% 
    dplyr::rename(`Wet House Hours` = wethousehours) %>%
    dplyr::rename(`Uniquely Impacted Addresses` = impacthousecount) %>%
    dplyr::rename(`Maximum Impact Depth` = maximumdepth) 
  xx$mapindex.point <- xx$mapindex.point %>% 
    dplyr::rename(`Index Label` = IndexLabel) %>%
    dplyr::rename(`Wet House Hours` = wethousehours) %>%
    dplyr::rename(`Uniquely Impacted Addresses` = impacthousecount) %>%
  ...
  xx$chart.addts <- xts::xts(x=unlist(xx$floodsum.addimpact[1:user.forecast.timesteps]),
                             order.by= as.POSIXct(xx$nwm.discharge.dateTimeZoneZ, tz = "UTC"))
  xx$chart.areats <- xts::xts(x=unlist(xx$floodsum.area[1:user.forecast.timesteps]),
                              order.by= as.POSIXct(xx$nwm.discharge.dateTimeZoneZ, tz = "UTC"))
  xx$chart.summary = cbind(xx$chart.addts, xx$chart.areats) 
  ...
  if(user.address.source == "OpenStreetMap Addresses") {
    addString <<- "OSM Addresses"
  } else if(user.address.source == "User Provided Addresses") {
    addString <<- "User Addresses"
  } else {
    addString <<- user.address.source
  }
...
basedataSERVER <- function(input, output, session) {
  ...
  basemap_reactive <- reactive({
    ...
    leaflet() %>%
      ...
      suppressWarnings(addRasterImage(x=xx$hand.grid,colors=xx$hand.grid.pal,layerId="hand",group="hand",project=FALSE,maxBytes=20*1024*1024)) %>%
      suppressWarnings(addRasterImage(x=xx$catch.grid,colors=xx$catch.grid.pal,maxBytes=20*1024*1024,layerId="catch",group="catch")) %>%
      suppressWarnings(addRasterImage(x=xx$flood.grid$ffreq,colors=xx$flood.grid.ffreqpal,layerId="Flooding",group="Flooding",project=FALSE,maxBytes=20*1024*1024)) %>%
  ...
  output$addressDataset <- DT::renderDataTable({
    DT::datatable(as.data.frame(xx$address.mapbook) %>% dplyr::select(address,`Index Label`,dataset),
  ...
  output$roadsDataset <- DT::renderDataTable({
    DT::datatable(as.data.frame(xx$road.point.mapbook) %>% dplyr::select(`Index Label`,name),
                  editable = FALSE, class = 'compact hover stripe order-column row-border stripe', options = list(scrollX = TRUE), selection = "single")
  })
  output$streamsDataset <- DT::renderDataTable({
    DT::datatable(as.data.frame(xx$flow.line) %>% dplyr::select(COMID,GNIS_NAME,StreamOrde),
                  editable = FALSE, class = 'compact hover stripe order-column row-border stripe', options = list(scrollX = TRUE), selection = "single")
  })
...
impactSERVER <- function(input, output, session) {
  ...
  output$summarytitleblock <- renderImage({
    tmpfile <- TitleBlock %>% magick::image_write(tempfile(fileext='jpg'), format = 'jpg')
    list(
      src = tmpfile,
      filetype = "image/jpg")
  }, deleteFile = FALSE)
  ...
  swiper_map_reactive <- reactive({
    ...
    leaflet() %>%
      ...
      addMapPane("indexlabels", zIndex = 426) %>%
      addPolygons(data=xx$aoi.shp,color=gridcolor,fillColor=aoiColor,weight=1,options=pathOptions(pane="bg"),group="Borders") %>%
      suppressWarnings(addRasterImage(x=xx$flood.grid[[1]],colors=xx$flood.grid.pal,layerId="Flooding",group="Flooding",project=FALSE,maxBytes=20*1024*1024)) %>%
      addPolylines(data=xx$flow.line,weight=1,options=pathOptions(pane="flow"),group="Flowlines") %>%
      # addMarkers(gages)
  ...
  mapshot_summary_map <- function() {
    ifelse(input$gridColor==TRUE,gridcolor <- "Black",gridcolor <- "White")
    ifelse(input$aoiColor==TRUE,aoiColor <- "Black",aoiColor <- "White")
    mybbox <- raster::extent(sf::st_buffer(x=xx$aoi.shp,dist=0.00005,nQuadSegs=1,endCapStyle="SQUARE",joinStyle="ROUND",mitreLimit=1,singleSide=FALSE))
  ...
  mapshot_index_map <- function(timestep,index,bboxobj){
    ...
    if(is.null(index)) {
      m = leaflet(height=1000, width=1000) %>%
        ...
        addMapPane("indexlabels", zIndex = 426) %>%
        addPolygons(data=xx$aoi.shp,color=gridcolor,fillColor=aoiColor,weight=1,options=pathOptions(pane="bg"),group="Borders") %>%
        suppressWarnings(addRasterImage(x=xx$flood.grid[[timestep]],colors=xx$flood.grid.pal,layerId="Flooding",group="Flooding",project=FALSE,maxBytes=20*1024*1024)) %>%
        addPolylines(data=xx$flow.line,weight=1,options=pathOptions(pane="flow"),group="Flowlines") %>%
  ...
  mapshot_swiper_map <- function(timestep,index){
    ...
    if(is.null(index)) {
      mybbox <- raster::extent(sf::st_buffer(x=xx$aoi.shp,dist=0.0002,nQuadSegs=1,endCapStyle="SQUARE",joinStyle="ROUND",mitreLimit=1,singleSide=FALSE))
      m = leaflet(height=1000, width=1000) %>%
        addProviderTiles(input$userbasemap, group = "Base Map") %>%
        addMapPane("bg", zIndex = 410) %>%
        # addMapPane("flood", zIndex = 412) %>%
      ...
      mybbox <- raster::extent(sf::st_buffer(x=xx$mapindex.poly[subset(xx$address.point, eval(parse(text = names(xx$flood.grid)[timestep]))>0),][xx$mapindex.poly[subset(xx$address.point, eval(parse(text = names(xx$flood.grid)[timestep]))>0),]$`Index Label`==index,]
                                             ,dist=0.00005,nQuadSegs=1,endCapStyle="SQUARE",joinStyle="ROUND",mitreLimit=1,singleSide=FALSE))
      m = leaflet(height=1000, width=1000) %>%
        addProviderTiles(input$userbasemap, group = "Base Map") %>%
        addMapPane("bg", zIndex = 410) %>%
        # addMapPane("flood", zIndex = 412) %>%
  ...
  swiperchartdata <- reactive({
    ...
    ifelse(input$uislidertime=="Zulu", 
           timeseriesOrder <- as.POSIXct(xx$nwm.discharge.dateTimeZoneZ, tz = "GMT"),
           timeseriesOrder <- as.POSIXct(xx$nwm.discharge.dateTimeZoneLocal, tz = Sys.timezone()))
    xx$chartXTS.addts <- xts::xts(x=unlist(xx$floodsum.addimpact[1:user.forecast.timesteps]), order.by= timeseriesOrder)
    xx$chartXTS.areats <- xts::xts(x=unlist(xx$floodsum.area[1:user.forecast.timesteps]), order.by= timeseriesOrder)
  ...
  output$mapindextable <- DT::renderDataTable({
    DT::datatable(as.data.frame(xx$mapindex.point[which(xx$mapindex.point$`Wet House Hours`>0),]) %>% dplyr::select(`Index Label`,`Wet House Hours`,`Uniquely Impacted Addresses`,`Maximum Impact Depth`), 
                  editable = FALSE, class = 'compact hover stripe order-column row-border stripe', options = list(scrollX = TRUE), selection = "single")
  })
  output$addressimpacts <- DT::renderDataTable({
    DT::datatable(as.data.frame(xx$address.mapbook[which(xx$address.mapbook$ffreq>0),]) %>% dplyr::select(address,`Index Label`,dataset), 
                  editable = FALSE, class = 'compact hover stripe order-column row-border stripe', options = list(scrollX = TRUE), selection = "single")
  })
  output$roadimpacts <- DT::renderDataTable({
    DT::datatable(as.data.frame(xx$road.point.mapbook[which(xx$road.point.mapbook$ffreq>0),]) %>% dplyr::select(`Index Label`,name), 
                  editable = FALSE, class = 'compact hover stripe order-column row-border stripe', options = list(scrollX = TRUE), selection = "single")
  })
  ...
  observeEvent(input$backbutton, {
    ...
    if( nrow(subset(xx$address.point, eval(parse(text = names(xx$flood.grid)[match(input$timeslider, timestepseries)]))>0))>0) {
      ...
      values$currentIndexAdds <- subset(xx$address.mapbook, eval(parse(text = names(xx$flood.grid)[match(input$timeslider, timestepseries)]))>0 & xx$address.mapbook$`Index Label`==values$currentIndexLabel) %>% dplyr::select(address,`Index Label`,dataset)
      values$currentIndexRoads <- subset(xx$road.point.mapbook, eval(parse(text = names(xx$flood.grid)[match(input$timeslider, timestepseries)]))>0 & xx$road.point.mapbook$`Index Label`==values$currentIndexLabel) %>% dplyr::select(`Index Label`,name)
      ...
      leafletProxy("swiper_map") %>%
        flyToBounds(as.numeric(mybbox@xmax),as.numeric(mybbox@ymin),as.numeric(mybbox@xmin),as.numeric(mybbox@ymax))
    } else {
  ...
  observeEvent(input$timeslider, {
    ...
    if(nrow(subset(xx$address.point, eval(parse(text = names(xx$flood.grid)[match(input$timeslider, timestepseries)]))>0))==0) {
      leafletProxy("swiper_map") %>%
        ...
        suppressWarnings(addRasterImage(x=xx$flood.grid[[match(input$timeslider, timestepseries)]],colors=xx$flood.grid.pal,layerId="Flooding",group="Flooding",project=FALSE,maxBytes=20*1024*1024)) %>%
        addLegend("bottomleft", title = "Flood Depth",pal = xx$flood.grid.pal, values = xx$flood.grid.val, group = "Flooding") 
  ...
  observeEvent(input$print, {
    ...
    TitlePageTitleBlock <- magick::image_annotate(TitlePageTitleBlock, format(file.mtime(xx$aoi.path),format='%m/%d/%Y'), font = 'Georgia', location = "+418+383", size = 20) %>% 
      ...
      magick::image_annotate(user.road.source, font = 'Georgia', location = "+627+410", size = 20) %>%
      magick::image_trim()
    mybbox <- suppressWarnings(suppressMessages(raster::extent(sf::st_buffer(x=xx$aoi.shp,dist=0.0001,nQuadSegs=1,endCapStyle="SQUARE",joinStyle="ROUND",mitreLimit=1,singleSide=FALSE))))
    ...
    boundrydatapoint <- suppressWarnings(suppressMessages(boundrydatapoly %>% sf::st_centroid()))
    ...
    ggsave(
      ...
      plot = ggplot() +
        annotation_custom(tableGrob(as.matrix(as.data.frame(xx$mapindex.point[which(xx$mapindex.point$`Wet House Hours`>0),]) %>% dplyr::select(`Index Label`,`Wet House Hours`,`Uniquely Impacted Addresses`,`Maximum Impact Depth`)), vp = viewport(width=10))) +
...
rm(list=ls()[! ls() %in% 
               c("xx","finalSERVER","finalUI","basemapchoices","DISCLAIMERString",'forecastZ','forecastT',
                 'launch.browser','bboxobj','apptitle','TitleBlock','values','user.forecast.gen',
                 'dyCSScoolWeb','dyCSScoolMin','dyCSScoolPrint','basedir','user.aoi.filepath',
                 'user.aoi.string','user.aoi.source','user.address.source','user.road.source','user.forecast.source',
                 'featurefile','user.output.choice','browser.chromeium','quiet','user.forecast.timesteps')])
quiet(gc())
