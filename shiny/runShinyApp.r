...
#user.forecast.timesteps <- as.numeric("6")
#user.forecast.file <- ""
#user.forecast.members <- as.list(1) 
#user.output.choice <- "BVOSMOTM" 
...
if (!file.exists(paste0(basedir, "/data/misc/zctas.shp"))) {
  ...
  # Create ZCTA
  zctasShape <- tigris::zctas(cb = TRUE)
...
xx = FloodMapping::getRawData(AOI,paste0(basedir,"/AOI/"),user.aoi.filepath)
...
if (!file.exists(paste0(basedir,"/AOI/",user.aoi.filepath,"/grid_rec.shp"))) {
  ...
  validOA <- openadds::oa_list() %>% 
    filter(str_detect(processed, paste0("/us/",tolower(cdlTools::fips(aoiCounties$STATEFP[1], to = "Abbreviation")))))
...
xx$nwm.discharge.dateTimeZone <- names(xx$nwm.discharge)[-1] %>% 
  stringr::str_sub(3, 21) %>%
  paste("UTC")

if(!SkipNWISFlows) {
  firstdate <- names(xx$nwm.discharge)[-1][1] %>% substr(3, 12)
  lastdate <-  names(xx$nwm.discharge)[-1][length(names(xx$nwm.discharge)[-1])] %>% substr(3, 12)
...
xx$flood.grid = FloodMapping::map_flood(hand.path = xx$hand.path,
                                        ...
                                        add = 0)
xx$flood.grid <- round(xx$flood.grid, digits = 2)
...
if(user.output.choice=="GIS_O") {
  ...
  writeRaster(xx$flood.grid, filename=names(xx$flood.grid), bylayer=TRUE,format="GTiff")
...
if(all(xx$address.point$ffreq==0,na.rm = TRUE) | user.output.choice=="basedata") {
  if(all(xx$address.point$ffreq==0,na.rm = TRUE)) { print("-- Alert: No addresses forecasted to be impacted, defaulting to base data viewer --") }
  xx$address.point <- sf::read_sf(xx$address.path)
  xx$address.mapbook = suppressWarnings(suppressMessages(sf::st_intersection(xx$mapindex.poly, xx$address.point)))
  xx$road.point.mapbook = suppressWarnings(suppressMessages(sf::st_intersection(xx$mapindex.poly, sf::st_cast(xx$road.line,"POINT"))))
  xx$flow.point.mapbook = suppressWarnings(suppressMessages(sf::st_intersection(xx$mapindex.poly, sf::st_cast(xx$flow.line,"POINT"))))
...
xx$chartdata <- data.frame(xx$floodsum.x,xx$floodsum.nwmts,xx$floodsum.area,xx$floodsum.addimpact,xx$nwm.discharge.dateTimeZone)
#add freq to swiper
...
for (i in 1:length(xx$nwm.timestep)) {
  xx$floodsum1.x[[i]] <- i
  xx$floodsum1.nwmts[[i]] <- xx$nwm.timestep[[i]]
  xx$floodsum1.area[[i]] <- raster::cellStats(xx$flood.grid[[i]]>0, 'sum', na.rm=TRUE)*raster::res(xx$flood.grid[[i]])[1]*raster::res(xx$flood.grid[[i]])[2]*1e-6
  xx$floodsum1.addimpact[[i]] <- sum(xx$address.point[[xx$nwm.timestep[[i]]]]>0, na.rm = TRUE)
  xx$floodsum1.roadimpact[[i]] <- sum(xx$road.point[[xx$nwm.timestep[[i]]]]>0, na.rm = TRUE)
...
xx$chart.addts<- timeSeries::timeSeries(xx$floodsum.addimpact[1:user.forecast.timesteps], as.POSIXct(xx$nwm.discharge.dateTime, tz = "UTC"), units = "Impacted Addresses")
xx$chart.areats <-timeSeries::timeSeries(xx$floodsum.area[1:user.forecast.timesteps], as.POSIXct(xx$nwm.discharge.dateTime, tz = "UTC"), units = "Square miles of innuncdated land")
...
# This app deployment format is less than ideal, but the other alternative I was able to implement
# was a disgusting file-replace using vbs like so...
    ...
    dashboardSidebar(
      sidebarMenu(id = "sidebar",
                  ...
                  radioGroupButtons(inputId="summaryviewlayer",
                                    ...
                                    choices=c("Wet House Hours","Count of Impacted Addresses","Maximum Impact Depth","Individual Points"),
                                    individual = TRUE,
                                    checkIcon = list(
